  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-8">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-lg font-semibold">納入先マスタ</h1>
        {isAdmin && (
          <Link
            href="/masters/delivery-destinations/new"
      ...
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr className="border-b border-neutral-300 text-left dark:border-neutral-700">
            ...
            <th className="py-2 pr-4">納入先名</th>
            <th className="py-2 pr-4">住所</th>
            <th className="py-2 pr-4">持込地区</th>
          ...
          {destinations?.map((destination) => (
            ...
            >
              ...
              <td className="py-2 pr-4">
                {destination.is_active ? '有効' : '無効'}
              </td>
              {isAdmin && (
                <td className="py-2 pr-4">
                  <Link
                    href={`/masters/delivery-destinations/${destination.id}`}
